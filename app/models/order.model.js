...
Order.getOrderByUserEmail = (userEmail, result) => {
  dbConn.query(
    "SELECT * FROM Orders WHERE userEmail=?",
    userEmail,
    (err, res) => {
      if (err) {
...
Order.createOrder = (orderData, result) => {
  dbConn.query(
    "INSERT INTO Orders (userEmail, addressNo) VALUES(?, ?)",
    [orderData.userEmail, orderData.addressNo],
    (err, res) => {
      if (err) {
...
Order.updateOrder = (userEmail, orderReqData, result) => {
  dbConn.query(
    ...
    [orderReqData.addressNo, orderReqData.orderDate, userEmail],
    (err, res) => {
      if (err) {
...
Order.deleteOrder = (userEmail, result) => {
  dbConn.query(
    "DELETE FROM Orders WHERE userEmail=?",
    userEmail,
    (err, res) => {
      if (err) {
...
Order.deleteOrderbyuserEmailandOrderNo = (userEmail, orderNo, result) => {
  dbConn.query(
    "DELETE FROM Orders WHERE userEmail=? AND orderNo=?",
    [userEmail, orderNo],
    (err, res) => {
      if (err) {
        console.log("Error while deleting item", err);
